...
\end{equation}
with $\sigma(\cdot)$ the softmax link and $p(f^*\mid\mathcal{D}_{k})$ the (non-Gaussian) latent predictive distribution. The integral is intractable, so we approximate the posterior by a Gaussian $q(\mathbf{f})$ via variational inference using GPyTorch~\cite{gardner2018gpytorch} with $M$ inducing points \todo{$M$ (co-author): read off the GPyTorch training config; if $M$ differs per task, state the range and put the per-task values in Appendix~A's implementation details.}; a calibration analysis of the resulting confidences is given in Appendix~\ref{app:ablation}.
