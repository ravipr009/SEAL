...
\textbf{REBUTTAL: 1. In practice due to noise there is always a gap of MI(X,Y) and H(Y) - we never use H(Y). We say that X\_min should be as informative for Y as the complete sensor state X\_raw. So our asssumption is that X\_raw (somehow) is enough to classify Y then we can say that X\_min is also enough.
2. Assumption 2 constrains us to use DS policies when any policy (Diffusion, preprogrammed, RL etc) will work as long as it mode invariant and goal reachable. So I think we should say that we assume mode invariance and goal reachability, then uunder finite perturbations we will complete the task (if f=L) }
