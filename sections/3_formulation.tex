...
\textbf{REBUTTAL: 1. In practice due to noise there is always a gap of MI(X,Y) and H(Y) - we never use H(Y). We say that X_min should be as informative for Y as the complete sensor state X_raw. So our asssumption is that X_raw (somehow) is enough to classify Y then we can say that X_min is also enough.
