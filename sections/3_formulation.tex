...
\label{as:obs}
The discrete mode $Y$ is identifiable from the raw grounded state $X_{\text{raw}}$ up to irreducible observation noise, making $\mathcal{X}_{\text{raw}}$ a sufficient representation. A reduced feature space $\mathcal{X}_{\min}$ is therefore considered instantaneously observable if it preserves the mode-relevant information of the raw space, satisfying $I(X_{\min}; Y) \approx I(X_{\text{raw}}; Y)$. Under this condition, the nominal localization function $L$ is well-defined on $\mathcal{X}_{\min}$ outside the inherent noise floor.
\end{assumption}


\begin{remark}[Feature-space vs.\ physical-space semantics]
\label{rem:aliasing}
Definitions~\ref{def:inv}--\ref{def:reach} are stated on $\mathcal{X}_{\min}$, and they coincide with their physical counterparts exactly under two conditions: (i) Assumption~\ref{as:obs} holds on the visited states, so that no two physically distinct situations with different modes alias to the same feature point; and (ii) every failure-relevant physical variable is represented in $F_{\min}$; a violation carried by a variable outside $F_{\min}$ is undetectable by construction. 

Condition~(ii) is not implied by the MI certificate (which only ensures $I(X_{\min}; Y) \approx I(X_{\text{raw}}; Y)$). Because the empirical MI is computed on \emph{success-only} demonstrations, a failure-indicating feature (e.g., payload presence in a task where no demonstration ever drops the payload) has near-zero variance and contributes nothing to the empirical MI. Dropping it causes no measured information loss, meaning mathematical minimization alone would prune it as redundant. This is why the semantic prior of Sec.~\ref{sec:feature_engineering} is prompted \emph{prescriptively}, to nominate failure-relevant features, and why such features must be retained through minimization even when their nominal MI contribution is negligible.
\end{remark}

\begin{assumption}[Primitive competence]
\label{as:prim}
Each primitive $\pi_m$ achieves geometric mode invariance and goal reachability: from any $\mathbf{x}\in R_m$ its closed-loop flow reaches the nominal guard $G_{m,m'}$ without exiting $R_m$ through a non-nominal boundary. This holds \emph{by construction} for a goal-directed dynamical system whose attractor lies in the successor's initiation set~\cite{burridge1999sequential}, and we assume it of whatever primitive (dynamical system, diffusion policy, RL, or preprogrammed skill) plays that role. The residual, physical component of invariance (e.g., ``payload retained'') is not controllable by $\pi_m$ and is precisely what we monitor.
\end{assumption}
...
\end{remark} The paper's algorithmic content is therefore aimed at enlarging the region where the hypothesis holds, safely, and Sec.~\ref{sec:experiments} demonstrates that, with $f$ so learned, the closed-loop system completes the task and recovers from perturbations as Proposition~\ref{prop:main} predicts. In deployment, Algorithm~\ref{alg:supervision} additionally relaxes the idealized instantaneous-crossing rule: a transition is accepted only when it is confident and persistent, which trades an adjustable dwell time against robustness to observation noise near guards.
